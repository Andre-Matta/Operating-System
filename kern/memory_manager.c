...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	//TODO: [PROJECT 2021 - [2] User Heap] allocateMem() [Kernel Side]
		...
		uint32 num_pages=ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;
					uint32 v_address=virtual_address;
					for(int i=0; i<num_pages;i++){
						int ret=pf_add_empty_env_page( e,v_address,0);
						if(ret==0){
							v_address+=PAGE_SIZE;
						}
					}
...
//1. Free ALL pages of the given range from the Page File
	uint32 num_pages=ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;
	uint32 v_address = ROUNDDOWN(virtual_address, PAGE_SIZE);
		for(int i=0; i<num_pages;i++){

			pf_remove_env_page(e,v_address);

			v_address +=PAGE_SIZE;
		}
		//2. Free ONLY pages that are resident in the working set from the memory
			uint32 virtual_add = ROUNDDOWN(virtual_address, PAGE_SIZE);
			struct WorkingSetElement *element;
			LIST_FOREACH(element, &(e->ActiveList))
			{
				if ((element->virtual_address >= virtual_add)
						&& (element->virtual_address
								< virtual_add + ROUNDUP(size, PAGE_SIZE))) {
					LIST_REMOVE(&e->ActiveList, element);
					/*unmap frame*/
					uint32 *ptr_page_table3 =NULL;
					struct Frame_Info* ptr_frame_info3 = get_frame_info(e->env_page_directory ,(void*)element->virtual_address , &ptr_page_table3 );
					unmap_frame(e->env_page_directory,
							(void*) element->virtual_address);
					ptr_frame_info3->references-=1 ;
					if (ptr_frame_info3->references == 0)
					      free_frame(ptr_frame_info3);
					element->empty = 1;
					LIST_INSERT_HEAD(&(e->PageWorkingSetList), element);

					if (LIST_SIZE(&(e->SecondList)) != 0) {
						struct WorkingSetElement* second_head_ptr = LIST_FIRST(
								&(e->SecondList));
						LIST_REMOVE(&(e->SecondList), second_head_ptr);
						pt_set_page_permissions(e, second_head_ptr->virtual_address,
						PERM_PRESENT, 0);
						LIST_INSERT_TAIL(&(e->ActiveList), second_head_ptr);
			...
			{
				//cprintf("va in second = %x \n" , element2->virtual_address);

				//if va found in second list
				if ((element2->virtual_address >= virtual_add)
						&& (element2->virtual_address
								< virtual_add + ROUNDUP(size, PAGE_SIZE))) {
					LIST_REMOVE(&e->SecondList, element2);
					LIST_INSERT_HEAD(&(e->PageWorkingSetList), element2);

					//unmap frame
					uint32 *ptr_page_table4 =NULL;
					struct Frame_Info* ptr_frame_info4 = get_frame_info(e->env_page_directory ,(void*)element2->virtual_address , &ptr_page_table4 );
					unmap_frame(e->env_page_directory,
							(void*) element2->virtual_address);
					ptr_frame_info4->references--;
					if (ptr_frame_info4 == 0)
					  free_frame(ptr_frame_info4);
					element2->empty = 1;
	...
	for(int j=0;j<num_pages;j++)
{
    ...
    uint32 frame_of_page_table = e->env_page_directory[PDX(va)] >> 12;
	...
	if(count==0){
		 //uint32* ptr_page_table2 = NULL;
		 struct Frame_Info* frame= to_frame_info(frame_of_page_table * 4096);
	     frame->references--;
