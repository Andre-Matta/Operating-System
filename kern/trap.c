...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	struct WS_List* active_list_ptr = &(curenv->ActiveList);
	struct WS_List* second_list_ptr = &(curenv->SecondList);
	struct WS_List* page_working_list_ptr = &(curenv->PageWorkingSetList);

	/*if there are elements in PageWorkingSetList then do (still space in page working set list)*/

	if (LIST_SIZE(page_working_list_ptr) != 0) {
		/* Check if the Active list is full, if true,
		 remove the last element then add it to the second chance list.*/
		if (curenv->ActiveListSize == LIST_SIZE(&(curenv->ActiveList))) {
			struct WorkingSetElement* last = LIST_LAST(active_list_ptr);
			LIST_REMOVE(active_list_ptr, last);
			LIST_INSERT_HEAD(second_list_ptr, last);
			pt_set_page_permissions(curenv, last->virtual_address, 0,
			PERM_PRESENT);
		}

		/*	Check if the fault_va in the Second chance list,
		 set its PRESENT bit to 1 then add it to the Active list*/
		bool is_in_2nd_list = 0;
		...
		LIST_FOREACH(element , &(curenv->SecondList))
		{
			if (element->virtual_address == fault_va) {
				...
				LIST_REMOVE(second_list_ptr, element);
				LIST_INSERT_HEAD(active_list_ptr, element);
				is_in_2nd_list = 1;
				break;
			}
		...
		if (is_in_2nd_list == 0) {
			...
			/* Pull an element from the PageWorkingSetList for the faulted page,
			 and add the fault_va in it*/
			struct WorkingSetElement* working_object = LIST_LAST(
					page_working_list_ptr);
			working_object->virtual_address = fault_va;
			LIST_REMOVE(page_working_list_ptr, working_object);
			LIST_INSERT_HEAD(active_list_ptr, working_object);
			...
			map_frame(curenv->env_page_directory, ptr_frame_info,
					(uint32*) fault_va, PERM_WRITEABLE | PERM_USER);

			/* If the page does not exist on page file, then CHECK if it is a stack page. If so this
			 means that it is a new stack page, add a new empty page with this faulted address to
			 page file*/
			int ret = pf_read_env_page(curenv, (void*) fault_va);
			if (ret == E_PAGE_NOT_EXIST_IN_PF
					&& (fault_va < USTACKBOTTOM || fault_va > USTACKTOP))
				panic("cannot acces this part 1");
			if (ret == E_PAGE_NOT_EXIST_IN_PF) {
	...
	/*replacement*/
		...
		/*check if the virtual address is in 2nd list*/
		bool is_in_2nd_list = 0;
		struct WorkingSetElement * element;
		LIST_FOREACH(element , &(curenv->SecondList))
		{
			if (element->virtual_address == fault_va) {
				pt_set_page_permissions(curenv, fault_va, PERM_PRESENT, 0);
				struct WorkingSetElement *tail_of_active_list = LIST_LAST(
						active_list_ptr);
				pt_set_page_permissions(curenv,
						tail_of_active_list->virtual_address, 0, PERM_PRESENT);
				LIST_REMOVE(second_list_ptr, element);
				LIST_REMOVE(active_list_ptr, tail_of_active_list);
				LIST_INSERT_HEAD(active_list_ptr, element);
				LIST_INSERT_HEAD(second_list_ptr, tail_of_active_list);
				is_in_2nd_list = 1;
				break;
			}
		}
		...
		if (is_in_2nd_list == 0) {
			/*Pick the LRU approximation victim from the tail of the second chance list.*/
			struct WorkingSetElement * victium_ptr = LIST_LAST(second_list_ptr);

			/*If the victim page was modified*/
			uint32 page_permissions = pt_get_page_permissions(curenv,
					victium_ptr->virtual_address);
			;
			if (page_permissions & PERM_MODIFIED) {
				...
				struct Frame_Info *ptr_frame_info = get_frame_info(
						...
						(void*) (victium_ptr->virtual_address),
						&ptr_page_table);
				int ret = pf_update_env_page(curenv,
						(void*) victium_ptr->virtual_address, ptr_frame_info);
				if (ret == E_PAGE_NOT_EXIST_IN_PF)
					cprintf("page doesnt exist in page file \n");

			}
			/* removing victim from 2nd list */
			LIST_REMOVE(second_list_ptr, victium_ptr);
			...
			/* Check if the Active list is full, if true,
			 remove the last element then add it to the second chance list.*/
			struct WorkingSetElement* last_active1 = LIST_LAST(active_list_ptr);
			pt_set_page_permissions(curenv, last_active1->virtual_address, 0,
			PERM_PRESENT);
			LIST_REMOVE(active_list_ptr, last_active1);
			LIST_INSERT_HEAD(second_list_ptr, last_active1);
			/* Pull an element from the PageWorkingSetList for the faulted page,
			 and add the fault_va in it*/
			struct WorkingSetElement* new = victium_ptr;
			new->virtual_address = fault_va;
			LIST_INSERT_HEAD(active_list_ptr, new);

			/* Allocate and map a frame for the faulted page.*/
			struct Frame_Info *ptr_frame_info1 = NULL;
			allocate_frame(&ptr_frame_info1);
			map_frame(curenv->env_page_directory, ptr_frame_info1,
					(uint32*) fault_va, PERM_WRITEABLE | PERM_USER);
			...
			if (ret == E_PAGE_NOT_EXIST_IN_PF
					&& (fault_va < USTACKBOTTOM || fault_va > USTACKTOP))
				panic("cannot acces this part 2");
